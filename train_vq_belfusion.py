...
def evaluate(cfg, pred_list_em, speaker_em, listener_em, epoch):
    ...
    metrics = {
        ...
        "FRDist": compute_FRD(cfg.validation_dataset.data_path, pred_list_em[:,0], listener_em), # FRDist (1) --> slow, ~3 mins
        "FRCorr": compute_FRC(cfg.validation_dataset.data_path, pred_list_em[:,0], listener_em), # FRCorr (2) --> slow, ~3 mins
...
def train(cfg, model, train_loader, optimizer, criterion, device):
    ...
    for batch_idx, (_, _, s_emotion, _, _, _, l_emotion, l_3dmm, l_reference) in enumerate(tqdm(train_loader)):
        ...
        total_loss = losses["loss"] + prediction['emb_loss']
        total_loss.backward()
        optimizer.step()
...
def main():
    ...
    n_epoch = cfg.trainer.epochs
    # beta = frange_cycle_linear(n_epoch, 0.0, 1.0, 4, 1.0)
    print('ffffffffffffffffffffffffffffff', len(train_loader)*1000)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for epoch in range(start_epoch, cfg.trainer.epochs):
        ...
        # cfg.loss.args.w_kld = np.float64(beta[iter]).item()
        criterion = partial(getattr(module_loss, cfg.loss.type), **cfg.loss.args)
