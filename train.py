...
# Train
def train(args, model, train_loader, optimizer, criterion, iterations):
    losses = AverageMeter()
    ...
    beta = frange_cycle_linear(max_iter)
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, _, _, _, _, listener_emotion, listener_3dmm, _) in enumerate(tqdm(train_loader)):
        ...
        loss = rec_loss+  + beta[iterations]*kld_loss + args.div_p * d_loss
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        iterations += 1
        train_loss, rec_loss, kld_loss, div_loss = train(args, model, train_loader, optimizer, criterion, iterations)
