...
def val(args, model, val_loader, criterion, render, binarize=False):
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, speaker_emotion, _, listener_video_clip, _, listener_emotion, _, listener_references) in enumerate(tqdm(val_loader)):
        if torch.cuda.is_available():
            ...
            speaker_emotion, listener_emotion, listener_references = speaker_emotion[:,:750].cuda(), listener_emotion[:,:750].cuda(), listener_references[:,:750].cuda()
        with torch.no_grad():
...
def main(args):
    ...
    if not os.path.exists(config_path): # args-based loading --> Trans-VAE by default
        val_loader = get_dataloader(args, args.split, load_audio=False, load_video_s=False, load_video_l=True, load_emotion_s=True, load_emotion_l=True, load_3dmm_l=True, load_ref=True)
        ...
        val_loader = get_dataloader(dataset_cfg, args.split, load_audio=False, load_video_s=False, load_video_l=False, load_emotion_s=True,
                                                    load_emotion_l=True, load_3dmm_s=False, load_3dmm_l=False, load_ref=True)
